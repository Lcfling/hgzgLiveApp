package com.weitao.vcloud.live.server;
...
import com.weitao.vcloud.live.server.entity.AddVideoResponseEntity;
import com.weitao.vcloud.live.server.entity.RoomInfoEntity;
import com.weitao.vcloud.live.server.entity.TranscodeResponseEntity;
import com.weitao.vcloud.live.server.entity.VideoInfoEntity;
import com.weitao.vcloud.live.util.sys.AndTools;
import com.netease.nim.uikit.common.http.NimHttpClient;
import com.netease.nim.uikit.common.util.log.LogUtil;
import com.netease.nim.uikit.common.util.string.MD5;
import com.netease.nim.uikit.common.util.string.Sring128;
...
public class DemoServerHttpClient {
    ...
    private static final String API_CHANGE_ROOM_STATUS = "/live/api.php?act=updateChannelStatus";//开播状态
    ...
    private static final String API_CREATE_ROOM = "/live/api.php?act=getLive";
    private static final String API_ENTER_ROOM = "/live/api.php?act=comeIn";
    private static final String API_LEAVE_ROOM = "/live/api.php?act=updateChannelStatus";
    private static final String API_SEND_GIFT = "/live/api.php?act=gift";
    ...
    private static final String RESULT_OWNER = "im_name";
    public static final String RESULT_LIVE_STATUS = "status";
    ...
    public void createRoom(Activity context, final DemoServerHttpCallback<RoomInfoEntity> callback){
        ...
        StringBuilder body = new StringBuilder();
        body.append(REQUEST_APPKEY).append("=").append(Preferences.getUserappkey()).append("&")
            .append(REQUEST_DEVICE_ID).append("=").append(AndTools.getDeviceId(context));
        String nonce=Sring128.getRandomString(128);
        String signature=MD5.Sha1(nonce+body+Preferences.getUserToken());
        headers.put("appkey", Preferences.getUserappkey());
        headers.put("signature", signature);//签名
        headers.put("nonce", nonce);//128位随机数
        ...
        NimHttpClient.getInstance().execute(url, headers, body.toString(), new NimHttpClient.NimHttpCallback() {
            ...
            public void onResponse(String response, int code, Throwable exception) {
                if (code != 200 || exception != null) {
                    LogUtil.e(TAG, "register failed : code = " + code + ", errorMsg = "
                            + (exception != null ? exception.getMessage() : "null appkey="+Preferences.getUserappkey()+" account="+DemoCache.getAccount()));
                    if (callback != null) {
                        callback.onFailed(code, exception != null ? exception.getMessage() : "null");
                    }
                    return;
                ...
                LogUtil.e(TAG, "register failed : code = " + code + ", errorMsg = "
                        + (exception != null ? exception.getMessage() : "null appkey="+Preferences.getUserappkey()+" account="+DemoCache.getAccount()));
                LogUtil.e(TAG, "respinse="+response);
                try {
                    JSONObject resObj = JSONObject.parseObject(response);
                    int resCode = resObj.getIntValue(RESULT_KEY_CODE);
                    if (resCode == RESULT_CODE_SUCCESS) {
                    ...
                    LogUtil.e(TAG,"JSONException1111");
                    callback.onFailed(-1, e.getMessage());
                } catch (Exception e){
                    LogUtil.e(TAG,"JSONException2222");
                    callback.onFailed(-1, e.getMessage());
                }
            }
        });
            ...
            public void onResponse(String response, int code, Throwable exception) {
                ...
                try {
                    ...
                    if (resCode == RESULT_CODE_SUCCESS) {
                        ...
                        LogUtil.e(TAG, "resObj  这里不是200");
                        String error = resObj.getString(RESULT_KEY_ERROR_MSG);
                        callback.onFailed(resCode, error);
                    }
                } catch (JSONException e) {
                    LogUtil.e(TAG, "resObj   JSONException");
                    callback.onFailed(-1, e.getMessage());
                } catch (Exception e){
                    LogUtil.e(TAG, "resObj   Exception e");
                    callback.onFailed(-1, e.getMessage());
                }
            }
        });
    ...
    public void anchorLeave(String roomId, final DemoServerHttpCallback<Void> callback){
        ...
        StringBuilder body = new StringBuilder();
        body.append(REQUEST_APPKEY).append("=").append(Preferences.getUserappkey()).append("&").append("status")
        .append("=").append(0);
        String nonce=Sring128.getRandomString(128);
        String signature=MD5.Sha1(nonce+body+Preferences.getUserToken());
        headers.put("appkey", Preferences.getUserappkey());
    ...
    * @roomid
    * */
    public void sendGift(String roomid,int giftId, final DemoServerHttpCallback<Void> callback){
        String url = DemoServers.apiServer() + API_SEND_GIFT;
        Map<String, String> headers = new HashMap<>(1);
        headers.put(HEADER_CONTENT_TYPE, "application/x-www-form-urlencoded; charset=utf-8");
        headers.put(HEADER_DEMO_ID, HEADER_DEMO_ID_VALUE);

        StringBuilder body = new StringBuilder();
        body.append(REQUEST_APPKEY).append("=").append(Preferences.getUserappkey()).append("&").append("giftid")
                .append("=").append(giftId)
        .append("&").append(REQUEST_ROOM_ID).append("=").append(roomid);
        String nonce=Sring128.getRandomString(128);
        String signature=MD5.Sha1(nonce+body+Preferences.getUserToken());
        headers.put("appkey", Preferences.getUserappkey());
        headers.put("signature", signature);//签名
        ...
        NimHttpClient.getInstance().execute(url, headers, body.toString(), new NimHttpClient.NimHttpCallback() {
            ...
            public void onResponse(String response, int code, Throwable exception) {
                if (code != 200 || exception != null) {
                    LogUtil.e(TAG, "response failed : code = " + code + ", errorMsg = "
                            + (exception != null ? exception.getMessage() : "null"));
                    callback.onFailed(code, exception.getMessage());
                    return;
                }
    ...
    status=1开启 0 关闭
    * */
    public void anchorAction(int Status, final DemoServerHttpCallback<Void> callback){
        String url = DemoServers.apiServer() + API_CHANGE_ROOM_STATUS;
        Map<String, String> headers = new HashMap<>(1);
        headers.put(HEADER_CONTENT_TYPE, "application/x-www-form-urlencoded; charset=utf-8");
        headers.put(HEADER_DEMO_ID, HEADER_DEMO_ID_VALUE);
        ...
        NimHttpClient.getInstance().execute(url, headers, body.toString(), new NimHttpClient.NimHttpCallback() {
            ...
            public void onResponse(String response, int code, Throwable exception) {
                if (code != 200 || exception != null) {
                    LogUtil.e(TAG, "通知服务器开始直播了 Action response failed : code = " + code + ", errorMsg = "
                            + (exception != null ? exception.getMessage() : "null"));
                    callback.onFailed(code, exception.getMessage());
                    return;
                }
